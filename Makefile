...
DIST_COMMON = README $(am__configure_deps) $(pininclude_HEADERS) \
	...
	$(srcdir)/config.h.in $(srcdir)/pinball-config.in \
	$(srcdir)/pinconfig.h.in $(top_srcdir)/configure AUTHORS \
	COPYING ChangeLog INSTALL NEWS depcomp libltdl/config/compile \
	libltdl/config/config.guess libltdl/config/config.sub \
	libltdl/config/depcomp libltdl/config/install-sh \
	libltdl/config/ltmain.sh libltdl/config/missing mkinstalldirs
...
CONFIG_HEADER = pinconfig.h config.h
...
X_LIBS = 
X_PRE_LIBS =  -lSM -lICE
abs_builddir = /tmp/tmp-rzr.tmp/var/lib/contrib.sh.dir/pinball/pinball
abs_srcdir = /tmp/tmp-rzr.tmp/var/lib/contrib.sh.dir/pinball/pinball
...
distclean-hdr:
	-rm -f pinconfig.h stamp-h1 config.h stamp-h2
...
TAGS: tags-recursive $(HEADERS) $(SOURCES) pinconfig.h.in config.h.in $(TAGS_DEPENDENCIES) \
		$(TAGS_FILES) $(LISP)
	set x; \
	here=`pwd`; \
	...
	list='$(SOURCES) $(HEADERS) pinconfig.h.in config.h.in $(LISP) $(TAGS_FILES)'; \
	unique=`for i in $$list; do \
	    if test -f "$$i"; then echo $$i; else echo $(srcdir)/$$i; fi; \
	  done | \
	  $(AWK) '{ files[$$0] = 1; nonempty = 1; } \
...
ctags: CTAGS
CTAGS: ctags-recursive $(HEADERS) $(SOURCES) pinconfig.h.in config.h.in $(TAGS_DEPENDENCIES) \
		$(TAGS_FILES) $(LISP)
	list='$(SOURCES) $(HEADERS) pinconfig.h.in config.h.in $(LISP) $(TAGS_FILES)'; \
	unique=`for i in $$list; do \
	    if test -f "$$i"; then echo $$i; else echo $(srcdir)/$$i; fi; \
...
all-am: Makefile $(SCRIPTS) $(HEADERS) pinconfig.h config.h
